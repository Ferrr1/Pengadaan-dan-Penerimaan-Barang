<div class="p-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
    ...
    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
        Daftar kelompok anggaran...
    </p>
    {{-- Table Start --}}
    <div>
        <form action="{{ route('kelAnggarans.index') }}" method="GET">
        ...
        <div class="pt-4 overflow-x">
                ...
                <table class="min-w-full leading-normal">
                    <thead class="dark:bg-gray-900 dark:text-white bg-gray-100 text-gray-600">
                        <tr>
                            ...
                            <th
                                class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-700 text-left text-xs font-semibold uppercase tracking-wider">
                                Kode Kelompok Anggaran
                            </th>
                            <th
                                class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-700 text-left text-xs font-semibold uppercase tracking-wider">
                                Nama Kelompok Anggaran
                            </th>
                            <th
                                class="px-5 py-3 border-b-2 border-gray-200 dark:border-gray-700 text-left text-xs font-semibold uppercase tracking-wider">
                                Aksi
                    ...
                    <tbody class="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">
                        @forelse ($kel_anggarans as $kel_anggaran)
                            <tr class="text-sm border-b-2 border-gray-200 dark:border-gray-700">
                                ...
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">{{ $kel_anggaran->kode_kel_anggaran }}</p>
                                </td>
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">{{ $kel_anggaran->nama_kel_anggaran }}</p>
                                </td>
                                <td class="px-5 py-5 flex gap-2 items-center justify-center">
                                    <x-primary-button class="p-2" x-data=""
                                        x-on:click="$dispatch('open-modal', `edit_modal_kel_anggaran{{ $kel_anggaran->id }}}`)">
                                        <x-eva-edit-2-outline class="w-5 h-5" />
                                    </x-primary-button>
                                    {{-- Delete Button --}}
                                    <x-danger-button class="p-2" x-data=""
                                        x-on:click="$dispatch('open-modal', `delete_modal_kel_anggaran{{ $kel_anggaran->id }}}`)">
                        ...
                        @empty
                            <tr>
                                <td colspan="6"
                                    ...
                                    Tidak ada data kelompok anggaran yang tersedia.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="d-flex justify-content-between">
                <div>
                    @if ($kel_anggarans instanceof \Illuminate\Pagination\LengthAwarePaginator)
                        {{ $kel_anggarans->links() }}
                    @endif
                </div>
            </div>
