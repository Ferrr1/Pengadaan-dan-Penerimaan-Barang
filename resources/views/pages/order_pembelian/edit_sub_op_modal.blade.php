@foreach ($subOrderPembelians as $subOrderPembelian)
    <x-modal name="edit_modal_sub_op{{ $subOrderPembelian->id }}}" :show="false">
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                Edit Sub Order Pembelian
            </h2>
            {{-- Table Start --}}
            <div>
                <div class="p-4 overflow-x-auto">
                    <form action="{{ route('subOrderPembelians.update', $subOrderPembelian->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div x-data="{
                            subAnggaranId: '{{ $subOrderPembelian->subPermintaanPembelian->id }}',
                            subAnggaranSatuanProduk: '{{ $subOrderPembelian->subPermintaanPembelian->produk->satuan->nama_satuan }}',
                            subOrderKodeBarang: '{{ $subOrderPembelian->subPermintaanPembelian->produk->kode_produk }}',
                            subOrderNamaProduk: '{{ $subOrderPembelian->subPermintaanPembelian->produk->nama_produk }}',
                            subAnggaranHarga: '{{ $subOrderPembelian->subPermintaanPembelian->harga_sub_permintaan_pembelian }}',
                            subOrderSpesifikasi: '{{ $subOrderPembelian->subPermintaanPembelian->spesifikasi_sub_permintaan_pembelian }}'
                        }"
                                ...
                                <div class="p-1">
                                    ...
                                    <div class="relative mt-1">
                                        <x-text-input id="ppn_sub_order_pembelian" class="block w-full pr-10"
                                            type="text" name="ppn_sub_order_pembelian" value="11" />
                                        <div
                                            class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                            <span
                            ...
                            <div class="grid grid-cols-2 gap-4 min-w-full overflow-hidden">
                                <div class="p-1">
                                    ...
                                    <x-text-input id="kuantitas_sub_order_pembelian" class="block mt-1 w-full"
                                        ...
                                        value="{{ $subOrderPembelian->kuantitas_sub_order_pembelian }}" />
                                </div>
                                <div class="p-1">
                                    ...
                                    <x-text-input id="catatan_sub_order_pembelian" class="block mt-1 w-full"
                                        ...
                                        value="{{ $subOrderPembelian->catatan_sub_order_pembelian }}" />
                                </div>
                            </div>
                        </div>
