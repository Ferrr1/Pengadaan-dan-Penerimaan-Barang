                ...
                <div class="inline-block min-w-full shadow rounded-md overflow-x-auto w-full">
                    <table id="tableForSearchSubAnggaran" class="min-w-full leading-normal">
                        ...
                        <tbody class="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">
                            @forelse ($subPermintaans as $subPermintaan)
                                <tr class="text-sm border-b-2 border-gray-200 dark:border-gray-700">
                                    <td class="px-5 py-5">
                                        <p class="whitespace-no-wrap">{{ $loop->iteration }}</p>
                                    </td>
                                    <td class="px-5 py-5">
                                        <p class="whitespace-no-wrap">{{ $subPermintaan->produk->kode_produk }}</p>
                                    </td>
                                    <td class="px-5 py-5">
                                        <p class="whitespace-no-wrap">{{ $subPermintaan->produk->nama_produk }}</p>
                                    </td>
                                    <td class="px-5 py-5">
                                        <p class="whitespace-no-wrap">
                                            {{ formatAngka($subPermintaan->kuantitas_sub_permintaan_pembelian) }}</p>
                                    </td>
                                    <td class="px-5 py-5">
                                        <p class="whitespace-no-wrap">
                                            {{ formatRupiah($subPermintaan->subAnggaran->harga_anggaran) }}
                                        </p>
                                    </td>
                                    <td class="px-5 py-5 flex gap-2 items-center justify-start">
                                        ...
                                        <x-primary-button class="p-1" x-data
                                            x-on:click="
                                        $dispatch('select-sub-anggaran', { id: '{{ $subPermintaan->id }}',
                                        kodeProduk: '{{ $subPermintaan->produk->kode_produk }}',
                                        namaProduk: '{{ $subPermintaan->produk->nama_produk }}',
                                        satuanProduk: '{{ $subPermintaan->produk->satuan->nama_satuan }}' ,
                                        nodetail: '{{ $subPermintaan->no_detail }}',
                                        satuan: '{{ $subPermintaan->produk->satuan->nama_satuan }}',
                                        harga: '{{ $subPermintaan->harga_sub_permintaan_pembelian }}',
                                        spesifikasi: '{{ $subPermintaan->spesifikasi_sub_permintaan_pembelian }}' });
