    ...
    <div>
        ...
        <div class="pt-4 overflow-auto">
            <div class="inline-block min-w-full shadow rounded-md overflow-x-auto w-full">
                <table class="min-w-full leading-normal">
                    ...
                    <tbody class="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">
                        @forelse ($order_pembelians as $order_pembelian)
                            <tr class="text-sm border-b-2 border-gray-200 dark:border-gray-700">
                                ...
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ $order_pembelian->nomor_op }}</p>
                                </td>
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ $order_pembelian->permintaanpembelian->anggaran->project->nama_project }}
                                    </p>
                                </td>
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ $order_pembelian->rekanan->nama_rekanan }}</p>
                                </td>
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ \Carbon\Carbon::parse($order_pembelian->tgl_op)->isoFormat('DD/MM/YYYY') }}
                                    </p>
                                </td>
                                <td class="px-5 py-5 flex gap-2 items-center justify-start">
                                    <a href="{{ route('orderPembelians.show', $order_pembelian->id) }}">
                                    ...
                                    <x-danger-button type="button" class="p-2" x-data=""
                                        x-on:click="$dispatch('open-modal', `delete_modal_order_pembelian{{ $order_pembelian->id }}}`)">
                        ...
                        @empty
                            <tr>
                                <td colspan="6"
                                    ...
                                    Tidak ada data order pembelian yang tersedia.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="d-flex justify-content-between">
                <div>
                    @if ($order_pembelians instanceof \Illuminate\Pagination\LengthAwarePaginator)
                        {{ $order_pembelians->links() }}
                    @endif
                </div>
            </div>
        </div>
