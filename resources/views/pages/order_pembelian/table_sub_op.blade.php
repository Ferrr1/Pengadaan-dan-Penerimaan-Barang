<div class="p-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
    <div class="flex justify-between">
        ...
        <div class="flex gap-2">
            @if ($canPrintPdf)
                <div class="mt-4">
                    <a href="{{ route('subOrderPembelians.report', [$order_pembelians->id]) }}">
                        <x-secondary-button class="px-4 py-2">Cetak Semua Sub Order</x-secondary-button>
                    </a>
                    {{-- <a href="{{ route('subOrderPembelians.report', [$order_pembelian->id, 'limit' => 10]) }}"
                        class="btn btn-secondary">
                        Cetak 10 Sub Order Teratas
                    </a> --}}
                </div>
            @endif
            <x-primary-button x-data="" x-on:click="$dispatch('open-modal', `sub_op_modal`)"
                class="mt-4 px-4 py-2">Tambah Sub Order</x-primary-button>
        </div>
    </div>
    {{-- Table Start --}}
    <div>
        ...
        <div class="pt-4 overflow-auto">
            <div class="inline-block shadow rounded-md overflow-x-auto w-full">
                <table class="min-w-full leading-normal">
                    ...
                    <tbody class="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">
                        @forelse ($subOrderPembelians as $subOrderPembelian)
                            <tr class="text-sm border-b-2 border-gray-200 dark:border-gray-700">
                                ...
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ $subOrderPembelian->subPermintaanPembelian->produk->kode_produk }}
                                    </p>
                                </td>
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ $subOrderPembelian->subPermintaanPembelian->produk->nama_produk }}
                                    </p>
                                </td>
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ formatAngka($subOrderPembelian->kuantitas_sub_order_pembelian) }}
                                    </p>
                                </td>
                                <td class="px-5 py-5">
                                    <span
                                        ...
                                        <span class="relative">
                                            {{ $subOrderPembelian->subPermintaanPembelian->produk->satuan->singkatan_satuan }}
                                        </span>
                                    </span>
                                </td>
                                <td class="px-5 py-5">
                                    <p class="whitespace-no-wrap">
                                        {{ formatRupiah($subOrderPembelian->total_sub_order_pembelian) }}
                                    </p>
                                </td>
                                <td class="px-5 py-5 flex gap-2 items-center justify-center">
                                    <x-primary-button type="button" class="p-2" x-data=""
                                        x-on:click="$dispatch('open-modal', `edit_modal_sub_op{{ $subOrderPembelian->id }}}`)">
                                        <x-eva-edit-2-outline class="w-5 h-5" />
                                    </x-primary-button>
                                    {{-- Delete Button Modal --}}
                                    <x-danger-button type="button" class="p-2" x-data=""
                                        x-on:click="$dispatch('open-modal', `delete_modal_sub_op{{ $subOrderPembelian->id }}}`)">
                        ...
                        @empty
                            <tr>
                                <td colspan="12"
                                    ...
                                    Tidak ada data sub order pembelian yang tersedia.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="p-1 flex justify-end">
                <div class="flex gap-4">
                    <div>
                        <x-input-label :value="__('Total OP')" />
                        <x-text-input class="block mt-1 w-full" type="text"
                            value="{{ formatRupiah($total_jumlah_harga) }}" disabled />
                    </div>
                    <div>
                        <x-input-label :value="__('PPN 11%')" />
                        <x-text-input class="block mt-1 w-full" type="text"
                            value="{{ formatRupiah($ppn_jumlah_harga) }}" disabled />
                    </div>
                    <div>
                        <x-input-label :value="__('Total With PPN')" />
                        <x-text-input class="block mt-1 w-full" type="text"
                            value="{{ formatRupiah($totalWithPPN) }}" disabled />
                    </div>
                </div>
            </div>
            <!-- Pagination Links -->
            <div class="d-flex justify-content-between">
                <div>
                    @if ($subOrderPembelians instanceof \Illuminate\Pagination\LengthAwarePaginator)
                        {{ $subOrderPembelians->links() }}
                    @endif
                </div>
            </div>
        </div>
